...
def noise_schedule(
    num_timesteps, beta_start=0.00001, beta_end=0.005, schedule="quadratic"
):
    if schedule == "linear":
...
class MLPDiffusionModel(nn.Module):
    def __init__(self, theta_dim, y_dim, hidden_dim=256, num_timesteps=500):
        ...
        self.layer1 = nn.Sequential(nn.Linear(in_dim, hidden_dim), nn.LeakyReLU(0.1))
    ...
    def sample(self, N, y_observed, temperature=0.8):
        with torch.no_grad():
            ...
            theta_samples = (
                torch.randn((N, self.theta_dim), device=device) * temperature
            )
            ...
            for t in reversed(range(self.num_timesteps - 1, -1, -1)):
                ...
                mu_t = (
                    theta_samples - (beta_t / torch.sqrt(1 - alpha_hat_t)) * noise_pred
                ) / torch.sqrt(alpha_t)
...
def diffusion_loss(model, theta_0, y, num_timesteps, mse_loss):
    ...
    theta_noisy = (
        torch.sqrt(alpha_hat_t) * theta_0 + torch.sqrt(1 - alpha_hat_t) * noise
    )
...
def train_diffusion_model(
    model, train_loader, val_loader, num_epochs=1000, patience=20
):
    ...
    for epoch in range(num_epochs):
        ...
        for theta_batch, y_batch in train_loader:
            ...
            optimizer.zero_grad()
            loss = diffusion_loss(
                model, theta_batch, y_batch, model.num_timesteps, mse_loss
            )
            loss.backward()
            optimizer.step()
